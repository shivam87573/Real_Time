...
function MainAppShell() {
  const { currentMessages, handleSendMessage, user, login } = useChat();
  ...
  // 🌟 PREMIUM & EXACT GLASSMORPHISM OVERLAY VIEW
  if (!user) {
    return (
      <div className="flex min-h-screen w-screen items-center justify-center bg-[#050714] px-4 font-sans relative overflow-hidden select-none">
        
        {/* 🔮 Background Abstract Glowing Spheres (Exactly as shown in your layout) */}
        ...
        <div className="flex flex-col items-center w-full max-w-lg z-10 animate-fade-in">
          ...
          >
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                {/* Input Container with User SVG Icon */}
                ...
                {/* Under-Input Example Subtext */}
                <div className="px-4">
                  <span className="text-[11px] text-slate-500 font-medium">e.g. shivam</span>
                </div>
              </div>
  ...
  return (
    <div className="h-screen w-screen bg-slate-950 flex flex-row items-stretch overflow-hidden font-sans antialiased text-slate-200">
      
      {/* Sidebar Container */}
      <div className="w-64 md:w-72 flex-shrink-0 h-full border-r border-slate-800 bg-[#0F172A]">
        <Sidebar />
      </div>
